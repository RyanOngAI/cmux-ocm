...
final class KeyboardShortcutSettingsFileStoreStartupTests: XCTestCase {
    ...
    func testSettingsFileParsesFileEditorSyntaxHighlighting() throws {
        ...
        try preservingDefaults(keys: [
            FilePreviewSyntaxHighlightingSettings.key,
            settingsFileBackupsDefaultsKey,
            importedManagedDefaultsKey
        ]) {
            defaults.removeObject(forKey: FilePreviewSyntaxHighlightingSettings.key)
            defaults.removeObject(forKey: settingsFileBackupsDefaultsKey)
            defaults.removeObject(forKey: importedManagedDefaultsKey)

            // Defaults to on until the config opts out.
            XCTAssertTrue(FilePreviewSyntaxHighlightingSettings.isEnabled(defaults: defaults))
            ...
            withExtendedLifetime(store) {
                XCTAssertFalse(defaults.bool(forKey: FilePreviewSyntaxHighlightingSettings.key))
                XCTAssertFalse(FilePreviewSyntaxHighlightingSettings.isEnabled(defaults: defaults))
            }
        }
    }
